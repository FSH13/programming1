 ...
 * Fleur
 * whodunit
 * convert de kleuren in een aanwijzing in de hoop de clue te kunnen lezen
************************************************/
...
int main(int argc, char *argv[])
{
    ...
    fwrite(&bf, sizeof(BITMAPFILEHEADER), 1, outputw);

    // write outfile's BITMAPINFOHEADER
    fwrite(&bi, sizeof(BITMAPINFOHEADER), 1, outputw);
    ...
    for (int i = 0, biHeight = abs(bi.biHeight); i < biHeight; i++)
    {
        ...
        for (int j = 0; j < bi.biWidth; j++)
        {
            ...
            fwrite(&triple, sizeof(RGBTRIPLE), 1, outputw);
        }

        // skip over padding, if any
        fseek(inputr, padding, SEEK_CUR);
