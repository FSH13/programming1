...
void init_board(void) {
  ...
  int row = 0;
  int column = 0;
  int tiles_numbers = d*d-1;

  for ( row = 0; row < d; row++)
  {
    for (column = 0; column < d; column++)
    {
	board[row][column] = tiles_numbers;
	tiles_numbers--;
    }
    column = 0;
  }
...
bool move(int tile) {
  ...
  while (board[row][column] != tile && board[row][column] <= d*d-1)
  {
    row++;
    column++;
  }
...
//  int column_tile = column;
  ...
  while (board[row][column] != '-' && board[row][column] <= d*d-1)
  {
    row++;
    column++;
  }
